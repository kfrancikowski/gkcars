    ...
    <form wire:submit.prevent="submit" class="space-y-4 text-gray-700">
        ...
        <div class="flex flex-wrap -mx-2 space-y-4 md:space-y-0">
            <div class="w-full px-2 md:w-1/2">
                <label class="block mb-1" for="rent_start_date">@lang('livewire\rental.rent_start_date')</label>
                <input wire:model="rent_start_date"
                    class="w-full h-10 px-3 text-base placeholder-gray-600 border rounded-lg focus:shadow-outline"
                    type="date" id="rent_start_date" required />
                @error('rent_start_date')<span class="text-xs text-red-700">{{ $message }}</span>@enderror
            </div>
            <div class="w-full px-2 md:w-1/2">
                <label class="block mb-1" for="rent_end_date">@lang('livewire\rental.rent_end_date')</label>
